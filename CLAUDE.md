...
- frontmatter 필수: `title`, `date`, `category`
- frontmatter 권장: `summary`, `basis`, `status`
- frontmatter 선택: `type`, `tags`

### frontmatter 필드 상세
| 필드 | 설명 | 예시 |
|---|---|---|
| `summary` | "한 줄로" 블록에 표시될 요약, 60자 내외 | `"2주택 +20%p가 +5%p로 내려갑니다."` |
| `basis` | 글 기준일 | `"2026년 8월 31일"` |
| `status` | 법안/제도 상태 | `"정부안 (국회 심의 중)"`, `"시행"`, `"개정예정"` |
| `type` | 글 유형 | `"column"` / `"case"` / `"해설"` |
- **summary·basis가 원문에 없으면 사용자에게 물어본다.**

### date 규칙
- 오늘 날짜(작업일)로 넣는다. 미래 날짜 금지.

### category 판단 (본문을 읽고 자동 판단)
- `chwideuk`(취득세) / `yangdo`(양도소득세) / `jeungyeo`(증여세) / `sangsok`(상속세) / `etc`(기타) / `guide`(이렇게 준비하세요)
- `guide`: 홈택스 화면 안내, 상담 준비물, 서류 발급 방법 등 **절차 가이드** 글에 사용. guide 글의 하단 다크 CTA는 자동으로 "준비가 되셨으면 / 카카오톡으로 자료를 보내주시면 이어서 진행할게요"로 표시됨.
...
### 본문 형식 규칙
- h2(`##`) 소제목에 번호를 붙이지 않기 — 사이트에서 CSS counter로 자동 번호(01, 02…)가 붙음
- 표는 마크다운 표 문법으로 작성
- 세율·이자율 등 개정될 수 있는 숫자에는 `(2026년 기준)` 처럼 기준 시점 명시
- 원문의 이모지는 제거하되 내용은 그대로 유지
- 따옴표·괄호와 붙은 강조(`**`)는 렌더링이 깨질 수 있으니, 변환 후 본문에 `**` 문자가 그대로 남아있는지 검사한다. 필요시 `<strong>` 태그로 대체한다.
- 표가 포함된 글은 모바일에서 가로 넘침이 발생할 수 있으므로, 사용자에게 모바일 미리보기를 권장하는 안내를 한다.

### 발행 문법 (본문에서 사용하는 특수 마크업)
- **주의 콜아웃**: `> [!주의] 내용` — 좌측 테라코타 선 + "주의" 라벨로 렌더됨. 면책고지·경고에 사용
- **표 각주**: 표 바로 다음 줄을 `기준:`으로 시작하면 작은 회색 각주 스타일로 렌더됨
- **숫자 강조**: 백틱(`` ` ``)으로 감싼 숫자+단위는 모노 폰트(JetBrains Mono) 굵게로 렌더됨. 세율, 금액 등에 활용
...
- 게시글 템플릿: `src/pages/blog/[...slug].astro`
